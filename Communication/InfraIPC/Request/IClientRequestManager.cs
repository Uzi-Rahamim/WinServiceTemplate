...
namespace Intel.IntelConnect.IPC.Request
{
    public interface IClientRequestManager
    {
        ...
        public IAsyncEnumerable<T> SendLongRequestAsync<T, R>(string methodName ,R message) where T : MessageHeader where R : MessageHeader;

        public Task<T?> SendRequestAsync<T, R>(string methodName, R message) where T : MessageHeader where R : MessageHeader;
