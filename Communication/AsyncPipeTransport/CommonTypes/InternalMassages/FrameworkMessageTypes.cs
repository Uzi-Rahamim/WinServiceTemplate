namespace AsyncPipeTransport.CommonTypes
{
    public partial class FrameworkMessageTypes
    {
        public static readonly string OpenSession = "___OpenSession";
        public static readonly string RequestSchema = "___RequestSchema";
        public static readonly string PulseEvent = "___PulseEvent";
        public static readonly string RegisterEvent = "___RegisterEvent";
        public static readonly string Echo = "___Echo";
        public static readonly string Empty = "___Empty";
        public static readonly string Error = "___Error";
    }
   
}
